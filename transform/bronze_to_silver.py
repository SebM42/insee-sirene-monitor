...
def read_batch(batch_date: str):
    ...
    # extraire la période active (dateFin IS NULL) de periodesEtablissement
    sdf_periods = sdf.withColumn(
        "active_period",
        F.filter("periodesEtablissement", lambda p: p["dateFin"].isNull())[0]
    )

    # aplatir les colonnes qui nous intéressent
...
def build_to_merge(sdf_batch, batch_date: str):
    ...
    # lignes silver des sirets affectés
    ...
    # vérification : sdf_joined doit avoir autant de lignes que sdf_batch
    ...
    # détecter changements sur PROJECT_HISTORIZED_COLS
    ...
    # détecter changements sur FIXED_COLS
    ...
    # calculer end_at pour la ligne à fermer
    ...
    # renommer toutes les colonnes de sdf_silver sauf siret avec suffixe _silver
    ...
    # sdf_base = sdf_joined x toutes les lignes silver (fermées incluses)
    sdf_base = sdf_joined.join(sdf_silver, on="siret", how="left")

    # appliquer FIXED_COLS sur toutes les lignes silver
    for col in FIXED_COLS:
    ...
    # fermer les lignes ouvertes avec changement significatif
    sdf_base = sdf_base.withColumn(
        "end_at_silver",
        F.when(
    ...
    # construire le DataFrame final depuis sdf_base (colonnes _silver → colonnes silver)
    ...
    ).filter(F.col("start_at").isNotNull())  # exclure nouveaux sirets (pas encore de ligne silver)

    # nouvelles lignes ouvertes pour sirets avec changement significatif
    ...
    # nouveaux sirets : dans sdf_joined avec start_at_silver IS NULL
...
def check_integrity(sdf_to_merge, sdf_batch, sdf_silver, sdf_joined) -> None:
    # vérifier qu'il n'y a pas de start_at > end_at
    ...
    # vérifier qu'on a bien une ligne ouverte par siret du batch
...
def run_bronze_to_silver() -> None:
    try:
        ...
        # pas de raise — on sort proprement sans retry
